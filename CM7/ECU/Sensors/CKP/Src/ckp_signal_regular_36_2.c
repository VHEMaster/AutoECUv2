...
#define CKP_SIGNAL_REGULAR_36_2_DELTA_TIMEOUT_MAX_US    (60 * TIME_US_IN_MS)
...
#define CKP_SIGNAL_REGULAR_36_2_DELTA_COUNT             4

#define CKP_SIGNAL_REGULAR_36_2_USPD_COUNT              8

typedef enum {
  CKP_SIGNAL_REGULAR_36_2_INDEX_0 = 0,
  CKP_SIGNAL_REGULAR_36_2_INDEX_1,
  CKP_SIGNAL_REGULAR_36_2_INDEX_MAX,
}ckp_signal_regular_36_2_index_t;
...
typedef struct {
    ...
    uint8_t delta_index;
    time_delta_us_t deltas[CKP_SIGNAL_REGULAR_36_2_DELTA_COUNT];
    time_delta_us_t delta_sum;
    uint8_t delta_count;

    time_delta_us_t abs_deltas[CKP_SIGNAL_REGULAR_36_2_ABS_INDEX_MAX];
    ...
    bool initial_found;
}ckp_signal_regular_36_2_runtime_index_ctx_t;

typedef struct {
    ckp_signal_regular_36_2_runtime_index_ctx_t indexed[CKP_SIGNAL_REGULAR_36_2_INDEX_MAX];

    time_delta_us_t rpm_deltas[CKP_SIGNAL_REGULAR_36_2_SIGNAL_INDEX_MAX];
    ...
    float uspd_array[CKP_SIGNAL_REGULAR_36_2_USPD_COUNT];
    float uspd_sum;
    uint8_t uspd_count;
    uint8_t uspd_index;
...
}ckp_signal_regular_36_2_runtime_ctx_t;

typedef struct {
    ckp_signal_regular_36_2_index_t level_index;
    ckp_signal_regular_36_2_index_t level_prev;

    ckp_signal_regular_36_2_runtime_ctx_t runtime;
}ckp_signal_regular_36_2_ctx_t;

static ckp_signal_regular_36_2_ctx_t ckp_signal_regular_36_2_ctx[CKP_INSTANCE_MAX];

error_t ckp_signal_regular_36_2_init(ckp_ctx_t *ctx, ckp_instance_t instance_index, void **usrdata)
{
  error_t err = E_OK;
  ckp_signal_regular_36_2_ctx_t **signal_ctx = (ckp_signal_regular_36_2_ctx_t **)usrdata;

  do {
    BREAK_IF_ACTION(ctx == NULL, err = E_PARAM);
    *signal_ctx = &ckp_signal_regular_36_2_ctx[instance_index];

    ckp_signal_regular_36_2_ctx[instance_index].level_prev = UCHAR_MAX;
  } while(0);
...
OPTIMIZE_FAST
ITCM_FUNC void ckp_signal_regular_36_2_signal(ckp_ctx_t *ctx, ecu_gpio_input_level_t level, void *usrdata)
{
  ckp_signal_regular_36_2_ctx_t * const signal_ctx = (ckp_signal_regular_36_2_ctx_t *)usrdata;
  ...
  do {
    ...
    switch(level) {
      case ECU_IN_LEVEL_LOW:
        index = CKP_SIGNAL_REGULAR_36_2_INDEX_0;
        break;
      case ECU_IN_LEVEL_HIGH:
        index = CKP_SIGNAL_REGULAR_36_2_INDEX_1;
        break;
      default:
    ...
    delta_mean_prev /= CKP_SIGNAL_REGULAR_36_2_DELTA_COUNT;
    ...
    if(++delta_index >= CKP_SIGNAL_REGULAR_36_2_DELTA_COUNT) {
      delta_index = 0;
    }

    if(++rpm_delta_index >= CKP_SIGNAL_REGULAR_36_2_SIGNAL_INDEX_MAX) {
      rpm_delta_index = 0;
    }

    if(++abs_delta_index >= CKP_SIGNAL_REGULAR_36_2_ABS_INDEX_MAX) {
      abs_delta_index = 0;
    }
    ...
    if(rpm_delta_count < CKP_SIGNAL_REGULAR_36_2_SIGNAL_INDEX_MAX) {
      rpm_delta_count++;
      signal_ctx->runtime.rpm_delta_count = rpm_delta_count;
    }

    abs_delta_count = signal_ctx->runtime.indexed[index].abs_delta_count;
    if(abs_delta_count < CKP_SIGNAL_REGULAR_36_2_ABS_INDEX_MAX) {
      abs_delta_count++;
      signal_ctx->runtime.indexed[index].abs_delta_count = abs_delta_count;
    }

    if(signal_ctx->runtime.indexed[index].delta_count < CKP_SIGNAL_REGULAR_36_2_DELTA_COUNT) {
      signal_ctx->runtime.indexed[index].delta_count++;
      break;
    }

    period_delta_sum = signal_ctx->runtime.rpm_delta_sum;
    if(signal_ctx->runtime.rpm_delta_count > 0 && signal_ctx->runtime.rpm_delta_count < CKP_SIGNAL_REGULAR_36_2_SIGNAL_INDEX_MAX) {
      period_delta_sum *= (float)CKP_SIGNAL_REGULAR_36_2_SIGNAL_INDEX_MAX / (float)rpm_delta_count;
    }
    ...
    if(data.validity < CKP_DATA_SYNCHRONIZED || signal_ctx->runtime.signal_index > CKP_SIGNAL_REGULAR_36_2_SIGNAL_INDEX_MAX - 2) {
    ...
    if(initial_found[0] && initial_found[1]) {
      ...
      if(data.validity >= CKP_DATA_SYNCHRONIZED) {
        if(signal_ctx->runtime.signal_index != CKP_SIGNAL_REGULAR_36_2_SIGNAL_INDEX_MAX) {
      ...
      data.current.position = ctx->config.signal_type_config.regular_36_2.zero_point;
    ...
    if(!initial_cur_found) {
      ...
      for(int i = 1; i < CKP_SIGNAL_REGULAR_36_2_INDEX_MAX; i++) {
    ...
    if(data.validity >= CKP_DATA_SYNCHRONIZED) {
      sync_index = signal_ctx->runtime.sync_signal_index;
      if(sync_index == index) {
        if(signal_ctx->runtime.signal_index >= 2 + CKP_SIGNAL_REGULAR_36_2_ABS_INDEX_MAX && signal_ctx->runtime.signal_index < CKP_SIGNAL_REGULAR_36_2_SIGNAL_INDEX_MAX - 4) {
          if(signal_ctx->runtime.indexed[index].abs_delta_count >= CKP_SIGNAL_REGULAR_36_2_ABS_INDEX_MAX &&
              signal_ctx->runtime.indexed[index_prev].abs_delta_count >= CKP_SIGNAL_REGULAR_36_2_ABS_INDEX_MAX) {
    ...
    if(sync_pos_updated) {
      ...
      if(data.previous.valid) {
        ...
        if(++uspd_index >= CKP_SIGNAL_REGULAR_36_2_USPD_COUNT) {
          uspd_index = 0;
        }

        if(uspd_count < CKP_SIGNAL_REGULAR_36_2_USPD_COUNT) {
          uspd_count++;
        }
  ...
  if(++index >= CKP_SIGNAL_REGULAR_36_2_INDEX_MAX) {
    index = 0;
  }
...
void ckp_signal_regular_36_2_loop_slow(ckp_ctx_t *ctx, void *usrdata)
{
  ...
  for(int i = 0; i < CKP_SIGNAL_REGULAR_36_2_INDEX_MAX; i++) {
    ...
    if(time_last != 0) {
      ...
      if(signal_ctx->runtime.rpm_delta_count > 16) {
        ...
        expected_delta *= CKP_SIGNAL_REGULAR_36_2_DELTA_TIMEOUT_MULT;
        expected_delta = MIN(expected_delta, CKP_SIGNAL_REGULAR_36_2_DELTA_TIMEOUT_MAX_US);
      } else {
        expected_delta = CKP_SIGNAL_REGULAR_36_2_DELTA_TIMEOUT_MAX_US;
      }
...
ITCM_FUNC void ckp_signal_regular_36_2_loop_fast(ckp_ctx_t *ctx, void *usrdata)
{
  ckp_signal_regular_36_2_ctx_t *signal_ctx = (ckp_signal_regular_36_2_ctx_t *)usrdata;
