...
#define CKP_SIGNAL_REGULAR_60_2_SIGNAL_POS_ADDER        6.0f
...
{
  ckp_signal_regular_60_2_ctx_t * const signal_ctx = (ckp_signal_regular_60_2_ctx_t *)usrdata;
  ...
  do {
    ...
    if(initial_found[0] && initial_found[1]) {
      ...
      data.current.position = ctx->config.signal_type_config.regular_60_2.zero_point;
    ...
    if(data.validity >= CKP_DATA_SYNCHRONIZED) {
      ...
      if(!sync_pos_updated) {
        if(signal_ctx->runtime.diff_sig_accept && signal_ctx->runtime.signal_index > 2) {
          if(signal_ctx->runtime.sync_signal_index == index) {
            pos_adder = CKP_SIGNAL_REGULAR_60_2_SIGNAL_POS_ADDER - CKP_SIGNAL_REGULAR_60_2_SIGNAL_POS_ADDER / (1.0f + signal_ctx->runtime.diff_sig_value);
          } else {
            pos_adder = CKP_SIGNAL_REGULAR_60_2_SIGNAL_POS_ADDER / (1.0f + signal_ctx->runtime.diff_sig_value);
          }
          sync_pos_updated = true;
          data_updated = true;
        } else if(signal_ctx->runtime.sync_signal_index == index) {
          pos_adder += CKP_SIGNAL_REGULAR_60_2_SIGNAL_POS_ADDER;
          sync_pos_updated = true;
          data_updated = true;
