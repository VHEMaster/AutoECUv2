...
error_t l9966_cfg_reg_translate_all(l9966_ctx_t *ctx)
{
  ...
  do {
    ...
    reg->data.status.bits.CALIB_SEL = config->status.calib_sel;
    ...
    reg->data.status.bits.CONFIG_CHECK = config->status.config_check;

    BREAK_IF_ACTION(config->status.overtemperature_mask >= L9966_CONFIG_GS_OTM_MAX, err = E_PARAM);
    reg->data.status.bits.OT_MASK = config->status.overtemperature_mask;
    ...
    for(int i = 0; i < ITEMSOF(config->curr_src_ctrl); i++) {
      if(i >= 4 || config->gtm_to_sent_route[i].pullup == L9966_CFG_GTSR_PU_OFF) {
        ...
        reg->data.curr_src_ctrl[i].bits.CIS = config->curr_src_ctrl[i].control_channel_selection;

        BREAK_IF_ACTION(config->curr_src_ctrl[i].comparator_threshold >= L9966_CFG_CSC_CT_MAX, err = E_PARAM);
        reg->data.curr_src_ctrl[i].bits.CTR = config->curr_src_ctrl[i].comparator_threshold;

        BREAK_IF_ACTION(config->curr_src_ctrl[i].current_source_value >= L9966_CFG_CSC_CSV_MAX, err = E_PARAM);
        reg->data.curr_src_ctrl[i].bits.CV = config->curr_src_ctrl[i].current_source_value;

        BREAK_IF_ACTION(config->curr_src_ctrl[i].csv_dewetting_phase >= L9966_CFG_CSC_CSV_DW_MAX, err = E_PARAM);
        reg->data.curr_src_ctrl[i].bits.CV_DW = config->curr_src_ctrl[i].csv_dewetting_phase;

        BREAK_IF_ACTION(config->curr_src_ctrl[i].pu_pd_sel >= L9966_CFG_CSC_PU_PD_SEL_MAX, err = E_PARAM);
        reg->data.curr_src_ctrl[i].bits.SEL = config->curr_src_ctrl[i].pu_pd_sel;

        BREAK_IF_ACTION(config->curr_src_ctrl[i].control_channel_invert >= L9966_CFG_CSC_CCP_INVERT_MAX, err = E_PARAM);
        reg->data.curr_src_ctrl[i].bits.MODE = config->curr_src_ctrl[i].control_channel_invert;

      } else if(config->gtm_to_sent_route[i].pullup == L9966_CFG_GTSR_PU_20K) {
        BREAK_IF_ACTION(config->curr_src_ctrl[i].comparator_threshold >= L9966_CFG_CSC_CT_MAX, err = E_PARAM);
        reg->data.curr_src_ctrl[i].bits.CIS = L9966_CFG_CSC_CSS_FORCE0;
        reg->data.curr_src_ctrl[i].bits.CTR = config->curr_src_ctrl[i].comparator_threshold;
        reg->data.curr_src_ctrl[i].bits.CV = L9966_CFG_CSC_CSV_0;
        reg->data.curr_src_ctrl[i].bits.CV_DW = L9966_CFG_CSC_CSV_DW_CSV;
        reg->data.curr_src_ctrl[i].bits.SEL = L9966_CFG_CSC_PU_PD_SEL_0;
        reg->data.curr_src_ctrl[i].bits.MODE = L9966_CFG_CSC_CCP_INVERT_FALSE;
      }
    }
    BREAK_IF(err != E_OK);

    BREAK_IF_ACTION(config->switch_route.aux_even_channel >= L9966_CFG_SR_AUX_EVENODD_CH_MAX, err = E_PARAM);
    reg->data.switch_route.bits.AUX_EVENCH = config->switch_route.aux_even_channel;

    BREAK_IF_ACTION(config->switch_route.aux_odd_channel >= L9966_CFG_SR_AUX_EVENODD_CH_MAX, err = E_PARAM);
    reg->data.switch_route.bits.AUX_ODDCH = config->switch_route.aux_odd_channel;

    BREAK_IF_ACTION(config->switch_route.sent_digital_filter_chx >= L9966_CFG_SR_DF_MAX, err = E_PARAM);
    reg->data.switch_route.bits.DIG_FILx = config->switch_route.sent_digital_filter_chx;

    BREAK_IF_ACTION(config->switch_route.aox_channel_src >= L9966_CFG_SR_AOX_CH_SRC_MAX, err = E_PARAM);
    reg->data.switch_route.bits.RAOX = config->switch_route.aox_channel_src;

    BREAK_IF_ACTION(config->switch_route.rsent_enabled_chx >= L9966_CFG_SR_RSENT_MAX, err = E_PARAM);
    reg->data.switch_route.bits.RSENTx = config->switch_route.rsent_enabled_chx;


    BREAK_IF_ACTION(config->dwt_volt_src_lsf_ctrl.lambda_enabled_chx >= L9966_CFG_DVSLC_LE_MAX, err = E_PARAM);
    reg->data.dwt_volt_src_lsf_ctrl.bits.LSF_MDx = config->dwt_volt_src_lsf_ctrl.lambda_enabled_chx;

    BREAK_IF_ACTION(config->dwt_volt_src_lsf_ctrl.dewetting_timer_time >= L9966_CFG_DVSLC_DTTC_MAX, err = E_PARAM);
    reg->data.dwt_volt_src_lsf_ctrl.bits.DWTC = config->dwt_volt_src_lsf_ctrl.dewetting_timer_time;

    BREAK_IF_ACTION(config->dwt_volt_src_lsf_ctrl.var_volt_source_sel >= L9966_CFG_DVSLC_VVSS_MAX, err = E_PARAM);
    reg->data.dwt_volt_src_lsf_ctrl.bits.VVAR_VF = config->dwt_volt_src_lsf_ctrl.var_volt_source_sel;
    ...
    reg->data.gtm_to_sent_route[0].bits.AN_FIL = config->gtm_to_sent_route[0].analog_filter == L9966_CFG_GTSR_AF_FAST ? 0x1 : 0;
    reg->data.gtm_to_sent_route[0].bits.AN_FIL |= config->gtm_to_sent_route[1].analog_filter == L9966_CFG_GTSR_AF_FAST ? 0x2 : 0;
    reg->data.gtm_to_sent_route[1].bits.AN_FIL = config->gtm_to_sent_route[2].analog_filter == L9966_CFG_GTSR_AF_FAST ? 0x1 : 0;
    reg->data.gtm_to_sent_route[1].bits.AN_FIL |= config->gtm_to_sent_route[3].analog_filter == L9966_CFG_GTSR_AF_FAST ? 0x2 : 0;
    ...
    reg->data.gtm_to_sent_route[0].bits.DIG_BP = config->gtm_to_sent_route[0].digital_bypass == L9966_CFG_GTSR_BP_ON ? 0x1 : 0;
    reg->data.gtm_to_sent_route[0].bits.DIG_BP |= config->gtm_to_sent_route[1].digital_bypass == L9966_CFG_GTSR_BP_ON ? 0x2 : 0;
    reg->data.gtm_to_sent_route[1].bits.DIG_BP = config->gtm_to_sent_route[2].digital_bypass == L9966_CFG_GTSR_BP_ON ? 0x1 : 0;
    reg->data.gtm_to_sent_route[1].bits.DIG_BP |= config->gtm_to_sent_route[3].digital_bypass == L9966_CFG_GTSR_BP_ON ? 0x2 : 0;
    ...
    reg->data.gtm_to_sent_route[0].bits.R20kPU = config->gtm_to_sent_route[0].pullup == L9966_CFG_GTSR_PU_20K ? 0x1 : 0;
    reg->data.gtm_to_sent_route[0].bits.R20kPU |= config->gtm_to_sent_route[1].pullup == L9966_CFG_GTSR_PU_20K ? 0x2 : 0;
    reg->data.gtm_to_sent_route[1].bits.R20kPU = config->gtm_to_sent_route[2].pullup == L9966_CFG_GTSR_PU_20K ? 0x1 : 0;
    reg->data.gtm_to_sent_route[1].bits.R20kPU |= config->gtm_to_sent_route[3].pullup == L9966_CFG_GTSR_PU_20K ? 0x2 : 0;

    BREAK_IF_ACTION(config->gtm_to_sent_route[0].sent_gtm_route >= L9966_CFG_GTSR_SGR_MAX, err = E_PARAM);
    reg->data.gtm_to_sent_route[0].bits.SENTo_GTM_ROUTE = config->gtm_to_sent_route[0].sent_gtm_route;
    BREAK_IF_ACTION(config->gtm_to_sent_route[1].sent_gtm_route >= L9966_CFG_GTSR_SGR_MAX, err = E_PARAM);
    reg->data.gtm_to_sent_route[0].bits.SENTe_GTM_ROUTE = config->gtm_to_sent_route[1].sent_gtm_route;
    BREAK_IF_ACTION(config->gtm_to_sent_route[2].sent_gtm_route >= L9966_CFG_GTSR_SGR_MAX, err = E_PARAM);
    reg->data.gtm_to_sent_route[1].bits.SENTo_GTM_ROUTE = config->gtm_to_sent_route[2].sent_gtm_route;
    BREAK_IF_ACTION(config->gtm_to_sent_route[3].sent_gtm_route >= L9966_CFG_GTSR_SGR_MAX, err = E_PARAM);
    reg->data.gtm_to_sent_route[1].bits.SENTe_GTM_ROUTE = config->gtm_to_sent_route[3].sent_gtm_route;


    BREAK_IF_ACTION(config->active_discharge_lsf.en_chx >= L9966_CFG_ADLC_EN_MAX, err = E_PARAM);
    reg->data.active_discharge_lsf_ctrl.bits.EN_CHx = config->active_discharge_lsf.en_chx;

    BREAK_IF_ACTION(config->active_discharge_lsf.time_cfg >= L9966_CFG_ADLC_TC_MAX, err = E_PARAM);
    reg->data.active_discharge_lsf_ctrl.bits.TIME_CFG = config->active_discharge_lsf.time_cfg;


    BREAK_IF_ACTION(config->wak_mask >= L9966_CFG_SC_WM_MAX, err = E_PARAM);
    reg->data.wak_msk.bits.MSK_INx = config->wak_mask;

    BREAK_IF_ACTION(config->sleep_config >= L9966_CFG_SC_SL_MAX, err = E_PARAM);
    reg->data.sleep_config.bits.TGT_INx = config->sleep_config;

    BREAK_IF_ACTION(config->sequencer_config.interrupt_mask_flag.int_eu1_pc_conf >= L9966_CFG_SQNCR_PC_MAX, err = E_PARAM);
    reg->data.sqncr_int_msk_flg.bits.INT_EU1_CONF = config->sequencer_config.interrupt_mask_flag.int_eu1_pc_conf;

    BREAK_IF_ACTION(config->sequencer_config.interrupt_mask_flag.int_eu2_pc_conf >= L9966_CFG_SQNCR_PC_MAX, err = E_PARAM);
    reg->data.sqncr_int_msk_flg.bits.INT_EU2_CONF = config->sequencer_config.interrupt_mask_flag.int_eu2_pc_conf;

    BREAK_IF_ACTION(config->sequencer_config.interrupt_mask_flag.interrupt_mask >= L9966_CFG_SQNCR_IM_MAX, err = E_PARAM);
    reg->data.sqncr_int_msk_flg.bits.CFGx = config->sequencer_config.interrupt_mask_flag.interrupt_mask;

    BREAK_IF_ACTION(config->vrs_config.double_edge_detection >= L9966_CFG_VRS_DED_MAX, err = E_PARAM);
    reg->data.reg_vrs.bits.LIM_ADAP_DOUBLE_EDGE = config->vrs_config.double_edge_detection;

    BREAK_IF_ACTION(config->vrs_config.diag_clr_cmd >= L9966_CFG_VRS_DCC_MAX, err = E_PARAM);
    reg->data.reg_vrs.bits.DIAG_CLR_CMD = config->vrs_config.diag_clr_cmd;

    BREAK_IF_ACTION(config->vrs_config.min_hyst_force >= L9966_CFG_VRS_MHF_MAX, err = E_PARAM);
    reg->data.reg_vrs.bits.MIN_HYST_FORCE = config->vrs_config.min_hyst_force;

    BREAK_IF_ACTION(config->vrs_config.vrs_mode_sel >= L9966_CFG_VRS_SEL_MAX, err = E_PARAM);
    reg->data.reg_vrs.bits.VRS_SEL = config->vrs_config.vrs_mode_sel;

    BREAK_IF_ACTION(config->vrs_config.conf_mode >= L9966_CFG_VRS_CM_MAX, err = E_PARAM);
    reg->data.reg_vrs.bits.VRS_CONF_MODE = config->vrs_config.conf_mode;

    BREAK_IF_ACTION(config->vrs_config.hyst_conf >= L9966_CFG_VRS_HM_MAX, err = E_PARAM);
    reg->data.reg_vrs.bits.VRS_HYST_CONF = config->vrs_config.hyst_conf;

    BREAK_IF_ACTION(config->vrs_config.diag_en >= L9966_CFG_VRS_DE_MAX, err = E_PARAM);
    reg->data.reg_vrs.bits.VRS_EN_DIAG = config->vrs_config.diag_en;

    BREAK_IF_ACTION(config->vrs_config.hyst_fb_mode >= L9966_CFG_VRS_HFBM_MAX, err = E_PARAM);
    reg->data.reg_vrs.bits.VRS_HYST_FB_MODE = config->vrs_config.hyst_fb_mode;

    BREAK_IF_ACTION(config->vrs_config.fe_filt_en >= L9966_CFG_VRS_FFE_MAX, err = E_PARAM);
    reg->data.reg_vrs.bits.VRS_FE_FILT_EN = config->vrs_config.fe_filt_en;


    BREAK_IF_ACTION(config->adc_timing.adc_volt_setting_time >= L9966_CFG_AT_AVST_MAX, err = E_PARAM);
    reg->data.adc_timing.bits.CT_AD = config->adc_timing.adc_volt_setting_time;

    BREAK_IF_ACTION(config->adc_timing.pu1_setting_time >= L9966_CFG_AT_PUST_MAX, err = E_PARAM);
    reg->data.adc_timing.bits.CT_PU1 = config->adc_timing.pu1_setting_time;

    BREAK_IF_ACTION(config->adc_timing.pu2_setting_time >= L9966_CFG_AT_PUST_MAX, err = E_PARAM);
    reg->data.adc_timing.bits.CT_PU2 = config->adc_timing.pu2_setting_time;

    BREAK_IF_ACTION(config->adc_timing.pu3_setting_time >= L9966_CFG_AT_PUST_MAX, err = E_PARAM);
    reg->data.adc_timing.bits.CT_PU3 = config->adc_timing.pu3_setting_time;
    ...
    reg->data.sqncr_ctrl.bits.INIT_PC_EU1 = config->sequencer_config.control.sqncr_cfg[0].init_pc;
    reg->data.sqncr_ctrl.bits.INIT_PC_EU2 = config->sequencer_config.control.sqncr_cfg[1].init_pc;
    reg->data.sqncr_ctrl.bits.SYNC_COPY_CMD_EN = config->sequencer_config.control.sync_copy_cmd_en;
    reg->data.sqncr_ctrl.bits.EU1_EN = 0;
    reg->data.sqncr_ctrl.bits.EU2_EN = 0;
    reg->data.sqncr_ctrl.bits.EU1_SYNC_EN = 0;
    reg->data.sqncr_ctrl.bits.EU2_SYNC_EN = 0;
    ...
    for(int i = 0; i < ITEMSOF(config->sequencer_config.cmd_config); i++) {
      ...
      reg->data.sqncr_cmd[i].bits.NXT_PC = config->sequencer_config.cmd_config[i].next_pc;

      BREAK_IF_ACTION(config->sequencer_config.cmd_config[i].r_volt_sel >= L9966_CFG_SQNCR_CMD_RVM_MAX, err = E_PARAM);
      reg->data.sqncr_cmd[i].bits.R_VOLT_MEAS_SELECT = config->sequencer_config.cmd_config[i].r_volt_sel;
