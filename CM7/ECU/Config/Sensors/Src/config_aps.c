...
static RAM_SECTION ecu_sensors_aps_ctx_t ecu_sensors_aps_ctx[ECU_SENSOR_APS_MAX] = {
    {
      .init = {
...
error_t ecu_sensors_aps_init(ecu_sensor_aps_t instance, aps_ctx_t *ctx)
{
  ...
  do {
    ...
    err = ecu_sensors_set_sensor_enabled(ECU_SENSOR_TYPE_APS, instance, false);
    BREAK_IF(err != E_OK);
...
error_t ecu_sensors_aps_configure(ecu_sensor_aps_t instance, const aps_config_t *config)
{
  ...
  do {
    ...
    err = ecu_sensors_set_sensor_enabled(ECU_SENSOR_TYPE_APS, instance, aps_ctx->ctx->config.enabled);
    BREAK_IF(err != E_OK);
