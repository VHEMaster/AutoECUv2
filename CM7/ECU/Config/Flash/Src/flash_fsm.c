      ...
      case FLASH_FSM_WRITE_PAYLOAD_SYNC:
        ...
        if(err == E_OK) {
          ...
          if(ctx->cmd_cur_len > 0) {
            ...
            ctx->fsm_process = FLASH_FSM_WRITE_MEMMAP_ENABLE;
          }
          continue;
        } else if(err != E_AGAIN) {
          ctx->cmd_errcode_internal = err;
        ...
        break;
      case FLASH_FSM_WRITE_MEMMAP_ENABLE:
        err = qspi_memory_mapping_set(ctx->qspi_ctx, true);
        if(err == E_OK) {
          ctx->fsm_process = FLASH_FSM_WRITE_MEMMAP_CRC_LOCK;
          continue;
        } else if(err != E_AGAIN) {
          ctx->cmd_errcode_internal = err;
          ctx->fsm_process = FLASH_FSM_FLASH_UNLOCK;
        ...
        break;
      case FLASH_FSM_WRITE_MEMMAP_CRC_LOCK:
        err = crc_lock();
        if(err == E_OK) {
          ctx->fsm_process = FLASH_FSM_WRITE_MEMMAP_CRC_HEADER;
          ctx->cmd_address = ctx->cmd_address_base;
          ctx->cmd_left_len = ECU_FLASH_SECTION_HEADER_LENGTH;
          continue;
        } else if(err != E_AGAIN) {
          ctx->cmd_errcode_internal = err;
          ctx->fsm_process = FLASH_FSM_WRITE_MEMMAP_DISABLE;
        }
        break;
      case FLASH_FSM_WRITE_MEMMAP_CRC_HEADER:
        err = crc_calculate(&ctx->checksum, (void *)(ctx->qspi_ctx->init.memory_base_address + ctx->cmd_address), OFFSETOF(flash_section_header_t, crc), true);
        if(err == E_OK) {
          ctx->fsm_process = FLASH_FSM_WRITE_MEMMAP_CRC_PAYLOAD;
          ctx->cmd_address += ECU_FLASH_SECTION_HEADER_LENGTH;
          ctx->cmd_left_len = ctx->cmd_length;
          continue;
        } else if(err != E_AGAIN) {
          ctx->cmd_errcode_internal = err;
          ctx->fsm_process = FLASH_FSM_WRITE_MEMMAP_CRC_UNLOCK;
        }
        break;
      case FLASH_FSM_WRITE_MEMMAP_CRC_PAYLOAD:
        err = crc_calculate(&ctx->checksum, (void *)(ctx->qspi_ctx->init.memory_base_address + ctx->cmd_address), ctx->cmd_length, false);
        if(err == E_OK) {
          ctx->fsm_process = FLASH_FSM_WRITE_MEMMAP_CRC_UNLOCK;
          ctx->cmd_left_len = 0;
          continue;
        ...
        break;
      case FLASH_FSM_WRITE_MEMMAP_CRC_UNLOCK:
        err = crc_unlock();
        if(err == E_OK) {
          ctx->fsm_process = FLASH_FSM_WRITE_MEMMAP_DISABLE;
          continue;
        ...
        break;
      case FLASH_FSM_WRITE_MEMMAP_DISABLE:
        err = qspi_memory_mapping_set(ctx->qspi_ctx, false);
        if(err == E_OK) {
          if(ctx->cmd_errcode_internal != E_AGAIN) {
            ctx->fsm_process = FLASH_FSM_FLASH_UNLOCK;
          } else {
            ctx->fsm_process = FLASH_FSM_WRITE_CHECKSUM_VERIFY;
          }
          continue;
        } else if(err != E_AGAIN) {
          if(ctx->cmd_errcode_internal != E_AGAIN) {
