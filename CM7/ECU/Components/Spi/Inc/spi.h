...
#ifndef COMPONENTS_SPI_INC_SPI_H_
#define COMPONENTS_SPI_INC_SPI_H_

#include "bool.h"

typedef void (*spi_op_cplt_cb_t)(spi_slave_t *spi, error_t errorcode);
...
error_t spi_transmit_and_poll(spi_slave_t *spi_slave, const void *transmit, uint16_t tx_bytes, const void *rx_mask, const void *rx_value, uint16_t rx_bytes, time_delta_us_t timeout);
error_t spi_transmit_byte_and_poll_byte(spi_slave_t *spi_slave, uint8_t transmit, uint8_t rx_mask, uint8_t rx_value, time_delta_us_t timeout);
...
#endif /* COMPONENTS_SPI_INC_SPI_H_ */
