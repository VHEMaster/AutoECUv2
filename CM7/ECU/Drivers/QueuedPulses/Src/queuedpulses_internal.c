...
INLINE void queuedpulses_internal_tim_enable(queuedpulse_timer_t *timer, uint32_t prd, uint32_t psc)
{
  while(prd > 0x10000) {
    prd >>= 1;
    psc <<= 1;
...
INLINE void queuedpulses_internal_tim_disable(queuedpulse_timer_t *timer)
{
...
INLINE time_delta_us_t queuedpulses_internal_calculate_pulse_cplt_time(queuedpulse_entry_t *entry, time_us_t now)
{
