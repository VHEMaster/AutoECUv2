...
STATIC_INLINE void calcdata_inputs_calc_iat(ecu_core_ctx_t *ctx)
{
  ...
  ecu_core_runtime_value_ctx_t iat_alpha_blending[ECU_BANK_MAX];
  ...
  bool data_iat_valid;
  float blending, blending_volume_sum;
  ...
  for(ecu_bank_t b = 0; b < banks_count; b++) {
    ...
    if(iat_value[b]->valid) {
      (void)core_calcdata_proc_get_output(ctx, b, CALCDATA_OUTPUT_IAT_ALPHA_BLENDING,
          iat_config[b]->alpha_blending.calc_variant, &iat_alpha_blending[b]);
      if(iat_alpha_blending[b].valid && ect_value[b]->valid && crankshaft->validity >= CKP_DATA_VALID) {
        blending = CLAMP(iat_alpha_blending[b].value, 0.0f, 1.0f);
        data_iat_valid = true;
        ...
        if(data_iat[b]->blending_valid) {
          for(ecu_config_calcdata_setup_iat_sensor_alpha_blending_filter_number_t f = 0;
            ...
            data_iat[b]->blending[f].filtered = BLEND(data_iat[b]->blending[f].unfiltered,
                data_iat[b]->blending[f].filtered, iat_config[b]->alpha_blending.filters[f].low_pass);
          }
        ...
        output_value[b].value = BLEND(iat_value[b]->value, ect_value[b]->value, blending);
        output_value[b].valid = true;
      } else {
        memcpy(&output_value[b], iat_value[b], sizeof(output_value[b]));
      }
    } else {
      memcpy(&output_value[b], iat_value[b], sizeof(output_value[b]));
    }
  ...
  for(ecu_bank_t b = 0; b < banks_count; b++) {
    (void)core_calcdata_proc_set_input(ctx, b,
        CALCDATA_RELATION_INPUT_SOURCE_CALC_IAT_MANIFOLD, &output_value[b]);
  }
