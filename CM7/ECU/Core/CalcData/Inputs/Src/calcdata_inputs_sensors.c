...
{
  const ecu_core_runtime_global_parameters_sensor_ctx_t *src;
  const ecu_core_runtime_value_ctx_t *src_read;
  ...
  if(src != NULL) {
    src_read = &src->read[0]; // TODO: set proper index
    ...
    if(src != NULL) {
      ...
      if(src_read->valid) {
        ...
        if(calc_model == CALCDATA_ECT_CALC_MODEL_MAXIMAL) {
          if(dest_value.value > src_read->value || value_count == 0) {
            dest_value.value = src_read->value;
          }
        } else if(calc_model == CALCDATA_ECT_CALC_MODEL_MEAN) {
          dest_value.value += src_read->value;
        } else if(calc_model == CALCDATA_ECT_CALC_MODEL_RMS) {
          tempval = src_read->value - rms_min;
          if(tempval > 0.0f) {
...
{
  const ecu_core_runtime_global_parameters_sensor_ctx_t **src_array;
  const ecu_core_runtime_global_parameters_sensor_ctx_t *src;
  const ecu_core_runtime_value_ctx_t *src_read;
  ecu_core_runtime_value_ctx_t *dest_ptr;
...
{
  const ecu_core_runtime_global_parameters_sensor_ctx_t *src;
  const ecu_core_runtime_value_ctx_t *src_read = NULL;
  ecu_core_runtime_value_ctx_t *dest_ptr;
...
STATIC_INLINE void core_calcdata_inputs_process_sensors_banked_iat(ecu_core_ctx_t *ctx, ecu_bank_t bank)
{
  ...
  if(type < ECU_CONFIG_IO_IAT_MAX && src_read != NULL) {
    memcpy(dest_ptr, src_read, sizeof(*dest_ptr));
  } else {
    memset(dest_ptr, 0, sizeof(*dest_ptr));
  }
